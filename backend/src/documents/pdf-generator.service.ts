...
export class PdfGeneratorService {
  /**
   * Helper to capture a PDFDocument stream into a Buffer
   */
  private streamToBuffer(doc: typeof PDFDocument.prototype): Promise<Buffer> {
   ...
   * Common Header
   */
  private drawHeader(doc: any, title: string, docCode?: string) {
    // Top Gold Accent Bar
  ...
  /**
   * 1. Pledge Agreement PDF
   */
  async generatePledgeAgreement(loan: any): Promise<Buffer> {
  ...
  /**
   * 2. Jewellery Annexure (Schedule A) PDF
   */
  async generateJewelleryAnnexure(loan: any): Promise<Buffer> {
    const doc = new PDFDocument({ margin: 40, size: 'A4' });
    const bufferPromise = this.streamToBuffer(doc);
  ...
  async generatePaymentReceipt(payment: any): Promise<Buffer> {
    const doc = new PDFDocument({ margin: 40, size: 'A5', layout: 'landscape' }); // A5 landscape receipt
    const bufferPromise = this.streamToBuffer(doc);
  ...
  /**
   * 4. Closure / Gold Release Voucher PDF
   */
  async generateClosureReceipt(loan: any): Promise<Buffer> {
    const doc = new PDFDocument({ margin: 40, size: 'A4' });
    const bufferPromise = this.streamToBuffer(doc);
    ...
    // Customer Handover Acknowledgement Box
